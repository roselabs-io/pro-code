...
I wanted to see the feedforward/feedback split as running code rather than a diagram, so I built a small pipeline in this repo, [pro-code](https://github.com/roselabs-io/pro-code). It runs **Frame → Plan → Implement**, and each phase hands off through a **grader** — a sensor, in Böckeler's terms. The one commitment it makes concrete is the split itself: each phase separates a **guide** (feedforward, domain-specific, swapped per profile) from a **grader** (feedback, agnostic). Two example services, both runnable, show it end to end — one where "done" means no cross-tenant data leak, one where it means no missed critical alert.
