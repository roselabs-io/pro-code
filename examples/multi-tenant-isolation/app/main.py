"""Request boundary — resolve a Caller, scope at the store, then gate on role.

The load-bearing order is SCOPE before ROLE: a foreign id 404s before any role check runs,
so a 403 is only ever returned for an in-tenant resource, never an existence oracle.
"""
...
app = FastAPI(title="multi-tenant-isolation")
app.add_exception_handler(ApiError, api_error_handler)  # type: ignore[arg-type]


def _deny_cross_tenant(caller: Caller, project_id: str) -> ApiError:
    """Log the denial if the id lives in another workspace, then 404 either way."""
...
def list_projects(
    ...
    """List the caller's own-workspace projects, cursor-paginated."""
...
def create_project(body: ProjectCreate, caller: Caller = Depends(get_caller)) -> Project:
    """Create a project owned by the caller's workspace."""
...
def get_project(project_id: str, caller: Caller = Depends(get_caller)) -> Project:
    """Read one scoped project; a foreign or missing id is 404."""
...
def update_project(
    ...
    """Patch a scoped project; an empty patch is 422, a foreign or missing id is 404."""
...
def delete_project(project_id: str, caller: Caller = Depends(get_caller)) -> Response:
    """Delete a scoped project; scope (404) is checked BEFORE the admin gate (403)."""
    project = store.get(caller.workspace_id, project_id)
    if project is None:
        raise _deny_cross_tenant(caller, project_id)
