"""Tenant-scoped store — every query carries a workspace_id, and it denies by default.

Isolation lives right HERE at the data line: a foreign id comes back None, never a row.
...
class ProjectStore:
    """In-memory projects, keyed by id, only ever touched through a workspace scope."""

    def __init__(self) -> None:
        self._items: dict[str, Project] = {}

    def create(self, workspace_id: str, name: str, description: str) -> Project:
        """Lay down a project owned by this workspace."""
        pid = uuid.uuid4().hex
        project = Project(
            id=pid,
    ...
    def update(
        ...
        """Patch a scoped project's fields; None if it don't live in this workspace."""
    ...
    def delete(self, workspace_id: str, project_id: str) -> bool:
        """Cut a scoped project loose; False if it don't live in this workspace."""
    ...
    def owner_of(self, project_id: str) -> str | None:
        """The workspace that owns an id — for the trace only, never on the record."""
        project = self._items.get(project_id)
    ...
    def list(
        ...
        """Hand back this workspace's projects as a stable cursor page — yours only."""
