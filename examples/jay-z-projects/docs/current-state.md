...
> **Profile:** `generic-saas` **+ the `personal/jay-z` overlay** (additive-only). The API is
> identical to `multi-tenant-isolation`; the overlay adds one requirement — every comment/docstring
> carries Jay-Z's voice **while staying comment-doctrine-clean** — checked by the `jay-z-voice` fuzzy
> grader (the composed fuzzy set is 4: feature · drift · docs-currency · jay-z-voice).
