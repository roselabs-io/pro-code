# jay-z-projects — Assumptions Ledger
...
>
> **Overlay note:** this build runs `generic-saas` + the `personal/jay-z` overlay. The overlay is
> additive-only — it adds the voice requirement + the `jay-z-voice` grader; it re-supplies no domain
> content, so the ledger below is the domain build's, unchanged by the overlay.
