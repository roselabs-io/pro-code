#!/usr/bin/env python3
"""Boundary-dependency codemod — make sure EVERY route carries `Depends(get_caller)`.

Isolation stands on every handler pulling a Caller at the boundary; a route that skips
the dependency is a hole with the door left open. This check (and its auto-fix) hold the
convention across all handlers so drift can't slide an unscoped route past.
...
def main(argv: list[str]) -> int:
    ...
    if check:
        if total_violations:
            print(f"require_caller_dep: {total_violations} unguarded route(s)")
            return 1
        print(
